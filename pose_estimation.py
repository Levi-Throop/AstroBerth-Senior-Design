...
python C:/Users/Parko/Desktop/Astroberth/Aruco/pose_estimation.py --K_Matrix C:/Users/Parko/Desktop/Astroberth/Aruco/calibration_matrix.npy --D_Coeff C:/Users/Parko/Desktop/Astroberth/Aruco/distortion_coefficients.npy --type DICT_4X4_100 --Aruco C:/Users/Parko/Desktop/Astroberth/Aruco/maze.txt
'''
...
if __name__ == '__main__':
    ...
    video = cv2.VideoCapture(1) # Use this to select webcam stream from OBS
    time.sleep(0.0)

    while True:
        ret, frame = video.read() # Read frames from video

        if not ret:
            break
        
        output = pose_estimation(frame, aruco_dict_type, k, d, maze_id_count) #Outputs coordinates

        cv2.imshow('Estimated Pose', output) #Displays Coordinates
        #double errorCorrectionRate = 0.4
        #tvec = tvec[0][0]
        #rvec = rvec[0][0]
        #location = str(ids[i]) + ", ".join(["{:.2f}".format(a) for a in tvec])
        #print(y)
        #cv2.imshow("Image", output)
        #cv2.imwrite("coords.txt", output)
